 ...
 * Checkbox item
 *
 * @version $Id: Checkbox.php,v 1.29 2004/09/24 15:15:48 steven Exp $
...
class PHAT_Checkbox extends PHAT_Element {
  ...
  function edit() {
    ...
    if(PHAT_SHOW_INSTRUCTIONS) {
      $editTags['INSTRUCTIONS'] = $_SESSION['translate']->it("Checkbox Element Instructions");
    }
  ...
  function save() {
    ...
    if((!$_SESSION['PHAT_FormManager']->form->checkLabel($_REQUEST['PHAT_ElementName']) && (strcasecmp($label, $_REQUEST['PHAT_ElementName']) != 0))
      ...
      $message = $_SESSION['translate']->it("The name you entered for the Checkbox is not valid or is already in use with this form.");
      $currentError = new PHPWS_Error("phatform", "PHAT_Checkbox::save()", $message, "continue", PHAT_DEBUG_MODE);
      $error = TRUE;
    }
	...
	return new PHPWS_Error("phatform", "PHAT_Checkbox::save()", $message, "continue", PHAT_DEBUG_MODE);
      }
    }
  } // END FUNC save
