 ...
 *
 * @version $Id: Menu.php,v 1.47 2003/12/15 19:31:23 steven Exp $
...
class PHPWS_Menu {
  ...
  function get_item_child($parent_id, &$level) {
    /* sql query */
    $level++;
    $sql = "SELECT * FROM " . PHPWS_TBL_PREFIX . "mod_menuman_items WHERE menu_item_pid='$parent_id' AND menu_item_id!=menu_item_pid AND menu_id='$this->menu_id' ORDER BY menu_item_order";
  ...
  function build_items() {
    ...
    $sql = "SELECT * FROM " . PHPWS_TBL_PREFIX . "mod_menuman_items WHERE menu_item_id=menu_item_pid AND menu_id='$this->menu_id' ORDER BY menu_item_order";
    $menu_item_result = $GLOBALS['core']->query($sql);
  ...
  function get_menu_display_child($expand, $parent_id, &$level, $display) {
    ...
    /* sql query */
    $sql = "SELECT menu_item_id FROM " . PHPWS_TBL_PREFIX . "mod_menuman_items WHERE menu_item_pid='$parent_id' AND menu_item_id!=menu_item_pid AND menu_id='$this->menu_id' ORDER BY menu_item_order";
  ...
  function build_display_menu($expand) {
    ...
    /* sql query */
    $sql = "SELECT menu_item_id FROM " . PHPWS_TBL_PREFIX . "mod_menuman_items WHERE menu_item_id=menu_item_pid AND menu_id='$this->menu_id' ORDER BY menu_item_order";
    $menu_item_result = $GLOBALS['core']->getcol($sql);
  ...
  function menu($action) {
    ...
    if(is_dir(PHPWS_HOME_DIR.$_SESSION['OBJ_menuman']->get_upload_directory())) {
      $image_options = PHPWS_File::readDirectory(PHPWS_HOME_DIR.$_SESSION['OBJ_menuman']->get_upload_directory(), FALSE, TRUE);
    } else {
...
/* ATO: inserted to select menu category via settings.tpl */
    $menuSettings['MENU_FATCAT'] = $_SESSION['OBJ_fatcat']->showSelect($this->menu_id, "multiple");
    $menuSettings['MENU_FATCAT_LABEL'] = $_SESSION["translate"]->it("Category");
/* ATO: end */
 ...
 /* ATO: save the selected categories */
/*  $_SESSION['OBJ_fatcat']->saveSelect($this->menu_title,'index.php?module=menuman&MMN_menuman_op=menuAction&MMN_editMenu=1&MMN_menu_id=' . $this->menu_id , $this->menu_id, NULL, 'menuman', NULL, NULL, TRUE); */
    $_SESSION['OBJ_fatcat']->saveSelect($this->menu_title,'', $max_menu_id, NULL, 'menuman', NULL, NULL, TRUE);
/* ATO: end */
 ...
 /* ATO: save the selected categories */
/*  $_SESSION['OBJ_fatcat']->saveSelect($this->menu_title,'index.php?module=menuman&MMN_menuman_op=menuAction&MMN_editMenu=1&MMN_menu_id=' . $this->menu_id , $this->menu_id, NULL, 'menuman', NULL, NULL, TRUE); */
    $_SESSION['OBJ_fatcat']->saveSelect($this->menu_title,'', $this->menu_id, NULL, 'menuman', NULL, NULL, TRUE);
/* ATO: end */
       }
    }
...
/* ATO: set activity in fatcat */
    if($this->active) {
      PHPWS_Fatcat::activate($this->id, "menuman");
    } else {
      PHPWS_Fatcat::deactivate($this->id, "menuman");
    }
/* ATO: end */
  }
  ...
  /* ATO: delete menu from fatcat */
        PHPWS_Fatcat::purge($this->menu_id, "menuman");
  /* ATO: end */
}
  ...
  function list_menu_items() {
	...
	$elements[0] .= PHPWS_WizardBag::js_insert("check_all", "MMN_menu_items") . "&#160;&#160;";
  ...
  function delete_menu_items() {
	...
	foreach($this->delete_items as $id) {
	  $sql = "SELECT menu_item_id FROM " . PHPWS_TBL_PREFIX . "mod_menuman_items WHERE menu_item_id='$id' AND menu_id='$this->menu_id' ORDER BY menu_item_order";
	  $menu_item_result = $GLOBALS['core']->query($sql);
	  if($menu_item_result->numrows() > 0) {
	    while($menu_item = $menu_item_result->fetchrow(DB_FETCHMODE_ASSOC)) {
	      $this->delete_item_child($menu_item['menu_item_id']);
