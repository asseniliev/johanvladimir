...
require_once(PHPWS_SOURCE_DIR."mod/xwysiwyg/class/xw_manager.php");
$this->content = "";
...
switch($action) {
	...
	if(isset($_REQUEST['editor']))
		PHPWS_xw_manager::toggleEditor_man(PHPWS_Text::parseInput($_REQUEST["editor"]));

	case "admin": //this for action=admin
 	$settings = PHPWS_xwysiwyg::readConfig();
	if(!is_array($settings)) {
		$this->content .= $settings;
		PHPWS_xw_manager::manageEditors();
		break;
	}
	PHPWS_xw_manager::showOptions_man($settings);
	break;

	case "testEditor":
	if(isset($_REQUEST['xw_testarea']))
		PHPWS_xw_manager::testEditors(PHPWS_Text::parseInput($_REQUEST['xw_testarea']));
	else {
		$text = "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.";
		PHPWS_xw_manager::testEditors($text);
	}
	break;
	...
	case "addEditor":
	PHPWS_xw_manager::addEditor();
	PHPWS_xw_manager::manageEditors();
	break;

	case "delEditor":
	PHPWS_xw_manager::delEditor();
	PHPWS_xw_manager::manageEditors();
	break;

	case "cpyEditor":
	PHPWS_xw_manager::cpyEditor();
	PHPWS_xw_manager::manageEditors();
	break;

	case "tarEditor":
	PHPWS_xw_manager::tarEditor();
	PHPWS_xw_manager::manageEditors();
	break;

	case "delFile":
	PHPWS_xw_manager::delFile();
	PHPWS_xw_manager::manageEditors();
	break;
