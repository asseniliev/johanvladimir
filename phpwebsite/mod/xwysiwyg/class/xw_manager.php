 ...
 * $Id: xw_manager.php,v 1.16 2005/11/18 00:02:32 ykuendig Exp $ *
...
function showOptions_man($settings) {
 if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg", "settings")) {
    if (isset($_REQUEST['func'])AND($_REQUEST['func'] == "update")) {
		$settings = PHPWS_xw_manager::requests_man($settings);
	}
	...
	$formTags['MESSAGE']	= PHPWS_xw_manager::updateOptions_man($settings);
    $formTags['TOGGLE']		= PHPWS_xw_manager::toggleButton_man($settings['editor']);
    $formTags['MANAGE']		= PHPWS_xw_manager::manageButton_man();
    $formTags['TEST']		= PHPWS_xw_manager::testButton_man();
	...
	$this->content .= PHPWS_xw_manager::showArea_man();
 } else {
...
function addEditor() {
	if ($_SESSION["OBJ_user"]->allow_access("xwysiwyg","settings")) {//Administrativ condition
		...
		if($GLOBALS['core']->sqlInsert($data, "mod_xw_config", TRUE, FALSE)) {
			...
			if(!$GLOBALS['core']->quickFetch("SELECT editor FROM mod_xw_editor", TRUE, TRUE))
				$result = PHPWS_xw_manager::toggleEditor_man($cfg_editor);
