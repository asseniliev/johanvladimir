 ...
 *
 * @version $Id: ImgLibrary.php,v 1.3 2004/10/04 12:39:29 adarkling Exp $
  ...
  function create_library () {
    ...
    if(!is_dir($the_path)) { 
      ...
      foreach(explode('/',$the_path) AS $k) {
        $a.=$k.'/';
        if(!is_dir($a) && !PHPWS_File::makeDir($a)) 
          exit('ERROR (PHPWS_IMGLib): Couldn\'t create directory "'.$a.'"<br />Make sure that the proper permissions are set in the parent directories.');
      } 
    }
    if(!is_dir($the_path)) 
      return false;
