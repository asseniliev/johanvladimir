...
class PHPWS_Fatcat extends PHPWS_Fatcat_Forms{
  ...
  function whatsRelated($module_id, $module_title = NULL){
	...
	continue;
 /* ATO: inserted to prevent display of menu's in whats related box */
      if ($get_element['module_title'] = 'menuman')
    continue;
 /* ATO: end */
  ...
  function savePic($postVar, $image_directory, $widthLimit=NULL, $heightLimit=NULL){
    ...
    echo phpws_debug::testarray($imageSize);
    ...
    if ($heightLimit && ($imageSize[1] > $heightLimit)){
      $this->error[] = $_SESSION["translate"]->it("Submitted image was too high").".";
      return FALSE;
    }
  ...
  function getModuleElementList($cat_id, $module_title){
    ...
    $pageIds = PHPWS_Array::paginateDataArray($idList, "index.php?".$_SERVER["QUERY_STRING"], 10, TRUE, NULL, NULL, 20, TRUE);
